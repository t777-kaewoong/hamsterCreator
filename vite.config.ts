...
export default defineConfig({
  ...
  server: {
    ...
    port: 5173,
    strictPort: true,
    // [흰 화면의 진짜 원인 — IPv6 전용 바인딩]
    // 이 값을 안 주면 vite는 'localhost'에 바인딩하는데, Node 17부터는 localhost가
    // 가리키는 주소를 재정렬하지 않아서 윈도우에서는 IPv6(::1)에만 붙고 IPv4
    // (127.0.0.1)에는 안 붙는 일이 생깁니다. 그러면 브라우저가 localhost를 127.0.0.1로
    // 풀어버릴 때 연결이 안 돼서 빈 화면만 보입니다.
    // (실제로 확인함: [::1]:5173 은 200, 127.0.0.1:5173 은 연결 실패)
    // host: true 는 IPv4·IPv6 양쪽에 다 붙게 합니다.
    // ※ 같은 공유기(교내망)에 있는 다른 기기에서도 접속할 수 있게 됩니다. 실행할 때
    //   터미널에 찍히는 "Network:" 주소를 태블릿·휴대폰에 입력하면 화면을 그대로
    //   볼 수 있어 편하지만, 그게 싫으면 이 줄을 host: '127.0.0.1' 로 바꾸세요
    //   (그러면 이 컴퓨터에서만 열립니다).
    host: true,
