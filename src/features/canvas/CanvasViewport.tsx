...
// ruler.ts·minimap.ts에 맡기고, 이 파일은 "언제 다시 그릴지"(리사이즈·팬·줌·문서 변경·
// 타일 로드 완료)와 "입력을 어떻게 받을지"(휠·드래그·스페이스바)만 담당합니다.
//
// 상호작용 오버레이(호버 표시, 스탬프 고스트, 선 긋기 미리보기 등)는 다음 단계에서 만듭니다.
// renderer.ts의 'overlay' 레이어는 이 단계에서 항상 빈 채로 남겨둡니다.
...
/** 지금 포커스가 글자 입력 요소에 있는지. 이럴 때는 Space를 눌러도 팬 모드로 들어가지
 *  않아야 합니다(예: 파일명 입력 중 스페이스는 그냥 띄어쓰기여야 함). */
...
export default function CanvasViewport() {
  ...
  // 마운트 시 한 번만: 렌더러·리사이즈 관찰·입력 이벤트를 전부 연결합니다.
  // 아래 함수들은 전부 ref를 통해 "최신" 값(doc, viewport 등)을 읽으므로, 이 effect 자체는
  // 의존성 없이 한 번만 실행해도 안전합니다(클로저 안에 오래된 값이 갇히지 않음).
  useEffect(() => {
    ...
    function drawMainLayer(name: LayerName, ctx: CanvasRenderingContext2D) {
      ...
      // 'props'와 'overlay'는 다음 단계에서 채워질 때까지 빈 채로 둡니다(그릴 것이 없음).
    }
    ...
    function endPanning(e: PointerEvent) {
      ...
      bodyWrap!.style.cursor = isSpaceDownRef.current ? 'grab' : 'default'
    }
    ...
    bodyWrap.addEventListener('pointerup', endPanning)
    bodyWrap.addEventListener('pointercancel', endPanning)
    bodyWrap.addEventListener('auxclick', preventAuxClick)

    // ── Space바를 누르고 있는 동안 커서를 grab으로(누르기 전엔 기본 커서) ────────────────────
    ...
    function handleKeyUp(e: KeyboardEvent) {
      ...
      if (!isPanningRef.current) bodyWrap!.style.cursor = 'default'
    }
    ...
    return () => {
      ...
      bodyWrap.removeEventListener('pointerup', endPanning)
      bodyWrap.removeEventListener('pointercancel', endPanning)
