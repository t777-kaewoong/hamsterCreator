...
export interface Prop {
  ...
  rot: number
  /** 좌우 반전 여부. PRD §5 원본 스키마에는 없었으나, FR-3.8(Shift 자유 배치)이 "지금
   *  스탬프의 방향(회전·반전)을 그대로 따른다"고 요구해 Cell.flip과 같은 의미로
   *  추가했습니다. 선택 필드라 이 필드가 없는 예전 파일도 그대로 열리고(false로 취급),
   *  parseMap도 props 배열 존재 여부만 검사해서 깨지지 않습니다(임의 확장 — PRD 갱신 필요). */
  flip?: boolean
}
