...
export interface TileDef {
  ...
  kind: TileKind
  /**
   * 이 그림을 격자선 **위**에 그릴지 여부.
   *
   * [왜 kind로 판단하지 않는가 — 2026-09-16]
   * 예전에는 `kind === 'object'`인 것만 선 위에 그렸습니다. 그래서 수풀·열매 수풀·눈 덮인
   * 바위처럼 칸 안에 세워져 있는 그림(kind: 'block')이 전부 검은 격자선에 반토막 난 채
   * 그려졌습니다(사용자가 수풀에서 발견). 그렇다고 block 전체를 위로 올리면 이번엔
   * 얼음 바닥·발자국처럼 이름 그대로 "바닥"인 것까지 올라가 격자선이 끊깁니다.
   * kind는 팔레트 묶음·영역 채우기용 모양 분류라서 레이어 순서와 1:1로 맞지 않습니다.
   * 그래서 타일마다 이 값을 manifest.json에 직접 적어 둡니다.
   *
   * ※ 선 위에 올린 그림은 그 칸의 검은 선을 가립니다. 로봇 바닥 센서가 그 지점에서 선을
   *   놓칠 수 있으니, 새 타일을 추가할 때 이 값을 true로 줄지 한 번 더 생각하세요.
   */
  aboveLine: boolean
